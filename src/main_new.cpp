...
{
    // cout<<fg.cols<<endl;
    //cv::resize(fg, fg, frame_clr.size(), 0, 0);
    //fg=Mat(frame_clr.size(), CV_8UC3, Scalar(0,0,0));
    bg=Mat(frame_clr.size(), CV_8UC3, Scalar(0,0,0));
    // cv::resize(bg, bg, frame_clr.size(), 0, 0);
  ...
  //	if(clipIsWhite==true)
    ...
    imshow("mask-in",obj_clr);
    cv::resize(fg, fg, frame_clr.size(), 0, 0);
  	namedWindow("mask", 0); imshow("mask", fg);
  	if(clipIsWhite==true) {

        cout<<bg.cols<<" "<<bg.rows<<endl;
        cout<<fg.cols<<" "<<fg.rows<<endl;
        cout<<frame_clr.cols<<" "<<frame_clr.rows<<endl;
