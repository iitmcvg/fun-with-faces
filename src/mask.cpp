...
int main()
{
	...
	cout<<"welcome to mask function :P\n";
  	//rectangle(frame_clr,Point(rx-rw/2,ry-rh/2),Point(rx+rw/2,ry+rh/2),Scalar(0, 0, 255));
	...
	RotatedRect rect=RotatedRect(Point(rx,ry),Size(rw,rh), angle);
			...
			corners[1]= Point2f(0,0);
			corners[2]= Point2f(obj_clr.cols-1, 0);
			corners[0]= Point2f(0, obj_clr.rows-1);
			corners[3]= Point2f(obj_clr.cols -1, obj_clr.rows -1);
